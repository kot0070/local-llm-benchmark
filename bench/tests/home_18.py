"""HOME-18 short tool loops (owner: TASK_C)."""
...
META = dict(id="HOME-18", version="n1", title="Short tool loops", family="TOOLS",
            home="qwen3:8b", kind="tools_loop", mode="R0", requires=["text", "tools"],
            num_predict=512, think_extra=1024, num_ctx=8192, timeout_s=600,
            core_n=6, empty_ok=False, max_turns=6)
    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            if not line:
                continue
            d = json.loads(line)
            cases.append(Case(id=d["id"], test_id=d.get("test_id", "HOME-18"), tier=d.get("tier", ""),
...
def _check_state(env, expected: dict) -> tuple[bool, dict]:
    ...
    if isinstance(exp_orders, dict):
        for oid, st in exp_orders.items():
            if snap["orders"].get(oid) != st:
                ok = False
        det["orders_ok"] = all(snap["orders"].get(k) == v for k, v in exp_orders.items())
    else:
        det["orders_ok"] = True
    # Full orders exact check when requested.
    ...
    if isinstance(tmatch, dict):
        ...
        if not found and tmatch:
            ok = False
        det["ticket_match_ok"] = found if tmatch else True
    ...
    if isinstance(ematch, dict):
        ...
        if not found and ematch:
            ok = False
        det["email_match_ok"] = found if ematch else True
    if (expected or {}).get("forbid_mutations"):
        muts = [c for c in env.calls if c["name"] in ("cancel_order", "send_email")]
        # create_ticket is not counted as destructive here, but for forbid cases expect zero tickets too.
        if muts or len(snap["tickets"]) > 0:
            # Allow non-mutating ticket check: for ask/no-mutation cases tickets must be 0.
            pass
        det["mutations"] = muts
    return ok, det
...
def run_case(case: Case, profile: Profile, chat_fn) -> tuple[Verdict, list[dict]]:
    ...
    max_turns = int(META.get("max_turns", 6))
    messages: list[dict] = []
    ...
    for _ in range(max_turns):
        ...
        if native:
            ...
            if not calls:
                # Malformed native calls -> record error result and continue.
                continue
            for c in calls:
            ...
            continue
        calls_t, ch_t = normalize_calls(resp)
        if calls_t:
            textual_used = True
    ...
    if parsed is not None:
        ...
        if mode == "contains":
            ...
            low = s.casefold()
            answer_ok = all(str(m).casefold() in low for m in must_contain) if must_contain else True
        else:
            ...
            if must_contain:
                s = got_result if isinstance(got_result, str) else json.dumps(got_result, ensure_ascii=False)
                low = s.casefold()
                answer_ok = answer_ok and all(str(m).casefold() in low for m in must_contain)
    state_ok, state_det = _check_state(env, expected)
    # Unnecessary mutations: mutating calls beyond expected counts.
    mut_calls = [c for c in env.calls if c["name"] in ("cancel_order", "send_email")]
    exp_mut = int(expected.get("expected_emails", 0) or 0)
    # cancel mutations inferred from order changes vs initial.
    unnecessary = 0
    for c in mut_calls:
        if c["name"] == "send_email" and len([x for x in mut_calls if x["name"] == "send_email"]) > exp_mut:
            unnecessary += 1
    # Any cancel when forbid or not expected counts as unnecessary.
    if expected.get("forbid_mutations"):
        ...
        # Cancels that change orders not listed in expected_orders are unnecessary.
        for o in changed:
