"""HOME-06 bug repair, hidden tests + BUG_LINE (owner: TASK_B1)."""
...
META = dict(
    id="HOME-06",
    version="n1",
    title="Bug repair with hidden tests and bug-line localisation",
    family="CODE",
    home="granite-code:8b-instruct",
    kind="chat",
    mode="R1",
    requires=["text"],
    num_predict=1024,
...
_BUG_LINE_RE = re.compile(r"(?im)^\s*BUG_LINE\s*:\s*(\d+)\s*$")
...
def parse_bug_line(text: str):
    m = list(_BUG_LINE_RE.finditer(text or ""))
    if not m:
        return None
    try:
...
def strip_bug_line(text: str) -> str:
    return _BUG_LINE_RE.sub("", text or "")


def load_cases(fixtures_dir: str) -> list[Case]:
    path = Path(fixtures_dir) / "cases.jsonl"  # runner passes fixtures/HOME-06
    if not path.is_file():  # tolerate the fixtures root as well
        path = Path(fixtures_dir) / "HOME-06" / "cases.jsonl"
    cases: list[Case] = []
...
def validate(case: Case, resp: Response, profile: Profile) -> Verdict:
    ...
    func = exp.get("func", "")
    tests = exp.get("tests", [])
    exp_line = exp.get("bug_line")
    bug_line = parse_bug_line(text)
    bug_line_correct = (bug_line is not None and bug_line == exp_line)
    code_text = strip_bug_line(text)
    code, info = extract_code(code_text, func)

    def _details(passed, total, failures):
        return {"passed": passed, "total": total, "failures": failures,
                "bug_line": {"parsed": bug_line, "expected": exp_line,
                             "correct": bug_line_correct},
                "fenced": info["fenced"], "prose": info["prose"]}

    if code is None:
        return Verdict(status=FORMAT_ERROR, sub_reason="NO_CODE", sem=0.0,
                       strict=0.0, details=_details(0, len(tests), []),
                       attribution="MODEL")
    try:
        compile(code, "<candidate>", "exec")
    except Exception as e:
        sub = "NO_BUG_LINE" if bug_line is None else "COMPILE_ERROR"
        status = FORMAT_ERROR if bug_line is None else WRONG_ANSWER
        return Verdict(status=status, sub_reason=sub, sem=0.0, strict=0.0,
                       details=_details(0, len(tests),
                                        [{"error": "COMPILE: %s" % e}]),
                       attribution="MODEL")
    ...
    format_broken = info["prose"] or bug_line is None
    if passed == total and total > 0 and not format_broken and bug_line_correct:
        return Verdict(status=OK, sub_reason=None, sem=1.0, strict=1.0,
                       details=_details(passed, total, []), attribution="NONE")
    if passed == total and total > 0:
        sub = ("NO_BUG_LINE" if bug_line is None
               else "WRONG_BUG_LINE" if not bug_line_correct
               else "PROSE_AROUND_CODE")
        return Verdict(status=FORMAT_ERROR, sub_reason=sub, sem=1.0,
                       strict=0.0, details=_details(passed, total, []),
                       attribution="MODEL")
    if format_broken:
        sub = "NO_BUG_LINE" if bug_line is None else "PROSE_AROUND_CODE"
        return Verdict(status=FORMAT_ERROR, sub_reason=sub, sem=sem,
                       strict=0.0, details=_details(passed, total, failures),
                       attribution="MODEL")
